...
export const FUNCTIONS = [
  ...
  // Substitui os triggers handle_new_tenant e handle_new_user_tenant. É Function, e não
  // código de front, porque só a API key cria Team e escreve em tenant_members
  // (deixar o cliente gravar o próprio papel seria deixar qualquer um virar owner).
  { name:'create-tenant', purpose:'Cria organização completa: Team nativo, adesão do criador, documento em tenants e tenant_members; ou o tenant pessoal do primeiro login', trigger:'http-frontend', cron:null, auth:'jwt-usuario', secrets:[], tables_read:['tenant_members','tenants'], tables_write:['tenants','tenant_members'], external:[], input:'{ name, slug?, logo_url? } | { personal:true }', output:'{ ok, tenant, created }', complexity:'baixa', notes:'Ordem Team -> adesão -> documento -> membro; falha em qualquer passo apaga o Team para não deixar órfão.' },
