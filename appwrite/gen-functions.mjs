...
export const FUNCTIONS = [
  ...
  { name:'google-calendar-auth', purpose:'OAuth2 do Google Calendar: authorize, callback e disconnect', trigger:'http-frontend', cron:null, auth:'jwt-usuario', secrets:['GOOGLE_CLIENT_ID','GOOGLE_CLIENT_SECRET','GOOGLE_TOKENS_ENCRYPTION_KEY'], tables_read:['google_calendar_tokens'], tables_write:['google_calendar_tokens'], external:['Google Calendar'], input:"querystring action/code/state (GET); { action:'disconnect' } (POST)", output:'HTML de sucesso/erro com postMessage, ou { success:true }', complexity:'alta', notes:'RPC encrypt_token (pgcrypto) não existe: cifre em Node com node:crypto. O REDIRECT_URI muda para a URL desta function e precisa ser recadastrado no Google Cloud Console.' },
  ...
  { name:'tenant-whatsapp-connect', purpose:'Cria a instância Evolution do WhatsApp corporativo do tenant e devolve o QR code', trigger:'http-frontend', cron:null, auth:'jwt-usuario', secrets:['EVOLUTION_API_URL','EVOLUTION_API_KEY'], tables_read:['tenant_members'], tables_write:['tenant_whatsapp_connections'], external:['Evolution API'], input:'{ tenant_id }', output:'{ ok, instance_name, qr_code }', complexity:'media', notes:'RPC get_tenant_role -> requireTenantAdmin() do _shared/auth.js.' },
  ...
  { name:'whatsapp-connect', purpose:'Cria a instância Evolution pessoal do usuário, registra o webhook e devolve o QR code', trigger:'http-frontend', cron:null, auth:'jwt-usuario', secrets:['EVOLUTION_API_URL','EVOLUTION_API_KEY','PUBLIC_WEBHOOK_BASE_URL'], tables_read:['whatsapp_connections'], tables_write:['whatsapp_connections'], external:['Evolution API'], input:'sem corpo (usa a sessão)', output:'{ status, qr_code, webhook_registered }', complexity:'media', notes:'A URL do webhook era montada com SUPABASE_URL. Agora vem de PUBLIC_WEBHOOK_BASE_URL + /whatsapp-webhook.' },
  ...
  { name:'whatsapp-status', purpose:'Consulta e sincroniza o estado da conexão WhatsApp pessoal com a Evolution', trigger:'http-frontend', cron:null, auth:'jwt-usuario', secrets:['EVOLUTION_API_URL','EVOLUTION_API_KEY'], tables_read:['whatsapp_connections'], tables_write:['whatsapp_connections'], external:['Evolution API'], input:'nenhum', output:'{ status, phone_number?, webhook_registered? }', complexity:'media', notes:'Compartilha o padrão de webhook/set com whatsapp-connect.' },
