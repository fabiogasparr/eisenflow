 ...
 * whatsapp-connect
 * ──────────────────────────────────────────────────────────────────────
 * Cria a instância Evolution pessoal do usuário, registra o webhook e devolve o QR code
 *
 ...
 * Entrada .......... sem corpo (usa a sessão)
 * Saída ............ { status, qr_code, webhook_registered }
 * Lê ............... whatsapp_connections
 * Escreve .......... whatsapp_connections
 * APIs externas .... Evolution API
 * Variáveis ........ EVOLUTION_API_URL, EVOLUTION_API_KEY, PUBLIC_WEBHOOK_BASE_URL
 * Complexidade ..... media
 *
 * ATENÇÃO NO PORTE:
 *   A URL do webhook era montada com SUPABASE_URL. Agora vem de PUBLIC_WEBHOOK_BASE_URL + /whatsapp-webhook.
 */
...
export default async ({ req, res, log, error }) => {
  try {
    ...
    // ──────────────────────────────────────────────────────────────────
    // PORTAR: a lógica de negócio vive em supabase/functions/whatsapp-connect/index.ts.
    ...
    log(`whatsapp-connect: recebido ${JSON.stringify(input).slice(0, 200)}`);

    return res.json({ ok: true, ported: false, message: 'esqueleto — portar a lógica de whatsapp-connect' });
  } catch (e) {
